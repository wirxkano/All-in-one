...
class Trainer:
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        self.model = my_model
        self.model_E = torch.nn.DataParallel(
            self.model.get_model().E, range(self.args.n_GPUs)
        )
        self.loss = my_loss
    ...
    def train(self):
        ...
        if epoch <= self.args.epochs_encoder:
            lr = self.args.lr_encoder * (
                self.args.gamma_encoder ** (epoch // self.args.lr_decay_encoder)
            )
        ...
        for key in keys:
            if "E.encoder_k" in key or "queue" in key:
                del model_dict[key]
        torch.save(
            model_dict, os.path.join(self.ckp.dir, "model", "model_{}.pt".format(epoch))
        )
            ...
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (hr, filename, _) in enumerate(self.loader_test):
                    ...
                    eval_ssim += utility.calc_ssim(
                        sr, hr, scale, benchmark=self.loader_test.dataset.benchmark
                    )
