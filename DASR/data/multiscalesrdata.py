...
class SRData(data.Dataset):
    def __init__(self, args, name="", train=True, benchmark=False):
        ...
        if args.ext.find("bin") >= 0:
            ...
            list_hr = self._scan()
            self.images_hr = self._check_and_load(args.ext, list_hr, self._name_hrbin())
        else:
            if args.ext.find("img") >= 0 or benchmark:
                ...
                for h in list_hr:
                    ...
                    self._check_and_load(args.ext, [h], b, verbose=True, load=False)
    ...
    def _scan(self):
        ...
        if not self.benchmark:
            with open(self.dir_hr, mode="r", encoding="utf-8") as f:
                names_hr = [line.strip() for line in f if line.strip()]
        else:
            names_hr = sorted(glob.glob(os.path.join(self.dir_hr, "*" + self.ext[0])))
        print(len(names_hr))
    ...
    def _name_hrbin(self):
        return os.path.join(self.apath, "bin", "{}_bin_HR.pt".format(self.split))

    def _name_lrbin(self, scale):
        return os.path.join(
            self.apath, "bin", "{}_bin_LR_X{}.pt".format(self.split, scale)
        )
        ...
        if self.train:
            ...
            for _ in range(2):
                hr_patch = common.get_patch(
                    hr, patch_size=self.args.patch_size, scale=scale
                )
                out.append(hr_patch)
        else:
            out = [hr]
