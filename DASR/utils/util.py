...
from degradation_model.model import DegradationModel
from degradation_model.configs import *
...
def anisotropic_gaussian_kernel(batch, kernel_size, covar):
    ...
    yy = (
        ax.repeat_interleave(kernel_size)
        .view(1, kernel_size, kernel_size)
        .expand(batch, -1, -1)
    )
    xy = torch.stack([xx, yy], -1).view(batch, -1, 2)

    inverse_sigma = torch.inverse(covar)
    kernel = torch.exp(-0.5 * (torch.bmm(xy, inverse_sigma) * xy).sum(2)).view(
...
def isotropic_gaussian_kernel(batch, kernel_size, sigma):
    ...
    kernel = torch.exp(-(xx**2 + yy**2) / (2.0 * sigma.view(-1, 1, 1) ** 2))

    return kernel / kernel.sum([1, 2], keepdim=True)


def random_anisotropic_gaussian_kernel(
    batch=1, kernel_size=21, lambda_min=0.2, lambda_max=4.0
):
...
def stable_anisotropic_gaussian_kernel(
    kernel_size=21, theta=0, lambda_1=0.2, lambda_2=4.0
):
...
def random_gaussian_kernel(
    ...
    if blur_type == "iso_gaussian":
        return random_isotropic_gaussian_kernel(
            batch=batch, kernel_size=kernel_size, sig_min=sig_min, sig_max=sig_max
        )
...
def stable_gaussian_kernel(
    ...
    if blur_type == "iso_gaussian":
        ...
        return stable_anisotropic_gaussian_kernel(
            kernel_size=kernel_size, lambda_1=lambda_1, lambda_2=lambda_2, theta=theta
        )
...
class bicubic(nn.Module):
    ...
    def cubic(self, x):
        ...
        f = (1.5 * absx3 - 2.5 * absx2 + 1) * condition1 + (
    ...
    def contribute(self, in_size, out_size, scale):
        ...
        indice0 = (
            left0.unsqueeze(1)
            + torch.arange(start=0, end=P).to(torch.float32).unsqueeze(0).cuda()
        )
        indice1 = (
            left1.unsqueeze(1)
            + torch.arange(start=0, end=P).to(torch.float32).unsqueeze(0).cuda()
        ...
        indice0 = torch.min(
            torch.max(torch.FloatTensor([1]).cuda(), indice0),
            torch.FloatTensor([in_size[0]]).cuda(),
        ).unsqueeze(0)
        indice1 = torch.min(
            torch.max(torch.FloatTensor([1]).cuda(), indice1),
            torch.FloatTensor([in_size[1]]).cuda(),
        ).unsqueeze(0)
    ...
    def forward(self, input, scale=1 / 4):
        ...
        weight0, weight1, indice0, indice1 = self.contribute(
        ...
        out = input[:, :, (indice0 - 1), :] * (
            weight0.unsqueeze(0).unsqueeze(1).unsqueeze(4)
        )
        out = torch.sum(out, dim=3)
        A = out.permute(0, 1, 3, 2)

        out = A[:, :, (indice1 - 1), :] * (
            weight1.unsqueeze(0).unsqueeze(1).unsqueeze(4)
        )
...
class BatchBlur(nn.Module):
    ...
    def forward(self, input, kernel):
        ...
        if len(kernel.size()) == 2:
            ...
            kernel = kernel.repeat(1, C, 1, 1).view(
...
class SRMDPreprocessing(object):
    def __init__(
        ...
        self.degrade = DegradationModel(SETTINGS)

    def __call__(self, hr_tensor, random=True, custom=True):
        with torch.no_grad():
            B, N, C, H, W = hr_tensor.size()
            if custom:
                hr_flat = hr_tensor.reshape(B * N, C, H, W)
                lr = self.degrade.random_shuffle_degradations(
                    hr_flat, scale_factor=0.25
                )
                _, _, H_lr, W_lr = lr.shape
                return lr.view(B, N, C, H_lr, W_lr)
            # only downsampling
            ...
            if self.mode == "bicubic":
                ...
                lr_blured = hr_blured.view(
                    -1, C, H // self.scale, self.scale, W // self.scale, self.scale
                )[:, :, :, 0, :, 0]
            ...
            if self.noise > 0:
                ...
                noise_level = (
                    torch.rand(B, 1, 1, 1, 1).to(lr_blured.device) * self.noise
                    if random
                    else self.noise
                )
                noise = (
                    torch.randn_like(lr_blured)
                    .view(-1, N, C, H_lr, W_lr)
                    .mul_(noise_level)
                    .view(-1, C, H_lr, W_lr)
                )
            ...
            return (
                lr_blured.view(B, N, C, H // int(self.scale), W // int(self.scale)),
                b_kernels,
            )
